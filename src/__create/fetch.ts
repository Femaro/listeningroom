...
const postToParent = (level: 'log' | 'info' | 'warn' | 'error', text: string, extra: unknown) => {
  try {
    if (isBackend() || !window.parent || window.parent === window) {
      const fn = level in console ? (console as Console & Record<'log' | 'info' | 'warn' | 'error', (...args: unknown[]) => void>)[level] : console.log;
      fn(text, extra);
      return;
    }
